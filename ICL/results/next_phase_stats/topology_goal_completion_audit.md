...
Generated: 2026-05-07. The hard-pilot evidence was added in commit `22de6ec` (`Add hard topology pilot evidence`). The branch now also includes follow-up orchestration, capacity-probe, and inference-verification updates: explicit submitted-job Python control, causal finalizer orchestration, guarded hard-pilot follow-up orchestration, tropical rooted-tree random-feature capacity fields, root-by-root tree-polytope support diagnostics, derived graph-family holdout, family-cluster bootstrap diagnostics, and a targeted next-phase report refresh utility for cluster follow-up artifacts.
...
- Local unit tests: `python3 -m unittest discover -s ICL/tests`, `130` tests passed.
...
| Replace `d_rel` with branch-margin/tree-polytope probes | A branch-separation capacity proxy or theory must be implemented and compared against `d_rel`. | `branch_margin_capacity.py` includes rank-weighted diagnostics, tropical rooted-tree random-feature diagnostics, and root-by-root tree-polytope common-rank support summaries; hard-regime branch-capacity regressions are reported. | Partial: useful lower-bound and rooted-polytope support probes exist, but this is not yet a solved tree-polytope capacity law. |
...
| Replace raw `d_rel` with branch-margin / tree-polytope capacity probes | `ICL/branch_margin_capacity.py`; `ICL/collect_branch_margin_capacity.py`; `ICL/results/*branch_margin_capacity*`; hard summaries in `ICL/results/expanded_hard_libraries/*/branch_margin_capacity_summary.json`; regression/report code now supports rooted-polytope predictor fields when capacity summaries are regenerated | Partial | Probe now includes rank-weighted fields, a tropical rooted-tree random-feature separability diagnostic, and root-specific tree-polytope common-rank support fields. These are stronger branch-geometry proxies, but still not a solved branch-margin/tree-polytope capacity theory. Current tracked hard summaries predate the rooted-polytope fields and should be refreshed on the cluster or regenerated locally from committed topology libraries. |
