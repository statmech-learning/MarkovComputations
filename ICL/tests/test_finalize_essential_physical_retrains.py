...
class FinalizeEssentialPhysicalRetrainsTests(unittest.TestCase):
    def make_experiment(self, tmpdir, selected_count=2, completed_count=0):
        root = os.path.join(tmpdir, "experiment")
        selected_dir = os.path.join(root, "essential_input50")
        ...
        with open(selected_csv, "w", newline="") as f:
            ...
            for idx in range(selected_count):
                writer.writerow(
                    {
                        "selected": "1",
                        "topology_id": f"ess{idx}",
                        "topology_name": f"essential_{idx}",
                        "edge_json": f"edge{idx}.json",
                    }
                )
            writer.writerow(
                {
        ...
        for idx in range(completed_count):
            ...
            run_dir = os.path.join(
                retrain_dir,
                f"ess{topology_idx}_trainseed{seed}",
            )
            os.makedirs(run_dir)
    ...
    def test_allow_partial_runs_dry_run_collection_commands(self):
        ...
        self.assertIn("--layout physical", result.stdout)
        self.assertNotIn("make_topology_research_report.py", result.stdout)

    def test_complete_retrains_finalize_without_report_generation(self):
        with tempfile.TemporaryDirectory() as tmpdir:
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("selected_subgraphs=2", result.stdout)
    ...
    def test_wrong_result_directories_do_not_satisfy_completion(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            wrong_dir = os.path.join(root, "essential_input50_retrain", "wrong_label")
        ...
        self.assertIn("ess0_trainseed1", result.stdout)
    ...
    def test_extra_result_directories_fail_without_allow_extra(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            extra_dir = os.path.join(root, "essential_input50_retrain", "stale_run")
    ...
    def test_selected_rows_require_topology_id(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with open(os.path.join(selected_dir, "selected.csv"), "w", newline="") as f:
                ...
                writer.writeheader()
                writer.writerow({"selected": "1", "topology_name": "essential_0"})
            result = self.run_finalizer(
                [
                    "--experiment",
                    f"exp={root}",
