"""Recover interrupted essential physical-subgraph retrain sweeps.

This is a conservative orchestration wrapper around the read-only auditor,
missing-only library submitter, and guarded physical finalizer. It is intended
for cluster recovery after SLURM arrays are interrupted or partially pending:
inspect source artifacts, emit status manifests, optionally resubmit only
missing retrain runs, and finalize only through the completion-checked
``finalize_essential_physical_retrains.py`` path.

The wrapper deliberately does not generate consolidated research reports.
After this succeeds, run ``make_topology_research_report.py`` explicitly for
the experiment set you want to interpret.
"""
...
def status_manifest_path(root):
    return os.path.join(root, "essential_input50_retrain", "task_manifest.csv")
...
def status_command(root, seeds):
    return python_script(
        ...
        "--library_csv",
        os.path.join(root, "essential_input50", "selected.csv"),
        "--output_root",
        os.path.join(root, "essential_input50_retrain"),
        "--seeds",
        seeds,
        "--status_only",
        "--manifest_csv",
...
def finalizer_command(experiments, seeds):
    return python_script(
        "finalize_essential_physical_retrains.py",
        *experiment_args(experiments),
        "--seeds",
        seeds,
    )
...
def main():
    ...
    for _, root in experiments:
        ...
        if not args.dry_run:
            ...
            print(f"Missing physical retrain tasks for {root}: {missing}/{total}")
        if args.submit_missing:
            if missing == 0:
                print(f"No missing physical retrain tasks for {root}; skipping missing-only submit")
                continue
            run_command(
    ...
    if args.finalize_if_complete:
        if submitted_missing:
            print(
                "Submitted missing physical retrain jobs; skipping finalization in this run. "
                "Rerun with --finalize_if_complete after jobs finish."
            )
            return
        run_command(finalizer_command(experiments, args.seeds), dry_run=args.dry_run)
        run_command(
            audit_command(
                experiments,
                args.seeds,
